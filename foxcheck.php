...
if (isset($_POST['uname']) && isset($_POST['password'])
	...
	if (empty($uname)) {
		...
		header("Location: changpass.php?error=Password must contain (1) Uppercase.&$user_data");
	    exit();
	}
	else if(!preg_match("#[a-z]+#",$pass)){
		header("Location: changpass.php?error=Password must contain (1) lowercase.&$user_data");
	    exit();
	}
